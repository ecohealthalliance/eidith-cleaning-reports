...
specimen <- read_tsv(h("raw-eidith-data", "Specimen.tsv.gz"), col_types = specimen_specs) %>%
  filter(Country == country) %>%
  select(-`TimeOfSpecimenCollection (humans only)`) # to revisit--having trouble getting into correct format

#class(specimen$`TimeOfSpecimenCollection (humans only)`) <- "hms"

## for excel-------

# check for NAs
na_index <- get_na(specimen, cols_to_ignore = c("Notes", "humans only"))
...
# check ID dups 
dup_id_index1 <- get_dups(specimen, "Animal/Human ID")
dup_id_index2 <- get_dups(specimen, "SpecimenID")
...
wb <- get_highlighted_wb(
  dfs = list(event, animal, specimen),
  tab.names = c("Event", "Animal", "Specimen"),
  markup.dfs = list(event.markup, animal.markup, specimen.markup)
)
