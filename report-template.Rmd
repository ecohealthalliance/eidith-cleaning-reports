...
-   *Values*: a summary of values that were entered into EIDITH
    ...
    -   **“All missing”** is written if a column has no values entered.
        ...
        “ClinicalSignsOther” is all missing. Whereas a column like
        Country should never be all missing.

-   *Count\_missing:* this is the number of rows that do not have a
    value for that column. In general, you will want to double check the
    columns that have missing values to confirm whether you need to make
    corrections. Counts with zeros in the numerator indicate no missing
    values.

    -   Sometimes it makes sense that you have mostly missing data. For
        example:

        -   If it says 165/165, then the values column will corroborate
            that and say “all missing”.

        -   If you only responded to one outbreak, count\_missing might
            be 160/165, which means that only 5 outbreak events were
            noted, which probably makes sense.

    -   In other cases, missing values point to an error. For example,
        if you have 2/165 missing from the Season column, you will need
        to make corrections for those 2 rows that are missing their
        Season.
...
-   Red means no value was entered in that row (it is missing):

    -   Please check to confirm whether you have data for that row to
        update EIDITH.

    -   Note, that if all of the cells are missing data, it is not
        highlighted here, but you can see that on the html sheet (where
        it will say all missing).
...
-   Purple means there is a duplicate value for something that should be
    unique (i.e., EventName, Animal\_ID, Specimen\_ID)
...
![](./media/image3.png){width="4.402777777777778in"
...
# check for NAs
na_index <- get_na(animal, cols_to_ignore = c("Notes", "Comment", "Taxa|Class|Order|Family|Genus|Species"))
