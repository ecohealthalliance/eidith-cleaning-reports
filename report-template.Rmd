...
for (i in seq_along(tables)){
  ...
  if(!is.null(tab)){
    metanames <- metadata %>% 
      ...
      pull(original_name)
  tab <- select(tab, SiteName, one_of("TaxaGroup"), !!metanames)# include taxa group if exists because needed in specimen table for human samples but marked as DROP
}
   cat('\n')  
   cat("<details>") 
   cat(paste0("<summary>", tab_name,  " Summary</summary>") )
   create_unique_table(tab, cols_to_ignore = c("Notes", "Comment", "SiteName", "ID", "EventName", "^Class$", "^Order$", "^Family$", "^Genus$", "Species")) %>% print()
   cat("</details>")
   cat('\n') 
}
```
