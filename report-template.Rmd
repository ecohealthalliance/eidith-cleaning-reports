...
# all tables
table_names <- c("Event", "Animal", "Specimen", "Human", "HumanAnimalProduction", "HumanCropProduction", "HumanExtractiveIndustry", "HumanHospitalWorker", "HumanHunter", "HumanMarket", "HumanRestaurant", "HumanSickPerson", "HumanTemporarySettlements", "HumanZoo")
...
tables <- map(table_names, function(tab){
  ...
  out <- try(read_tsv(h("raw-eidith-data", paste0(tab, "-", country, ".tsv.gz")),  col_types = get(paste(tolower(tab), "specs", sep="_"))), silent=TRUE)
    ...
    # select column names from metadata
  cnames <- metadata %>% 
    filter(endpoint2 == tab, original_name %in% names(out)) %>%
    pull(original_name)
  out <- try(select(out, SiteName, !!cnames), silent = TRUE)
...
for (i in seq_along(tables)){
  ...
  tab_name <- table_names[i]
   ...
   create_unique_table(tab, cols_to_ignore = c("Notes", "Comment", "SiteName", "ID", "EventName", "Taxa|Class|Order|Family|Genus|Species")) %>% print()
   cat("</details>")
   cat('\n') 
}
```
...
markup <- map(seq_along(tables), function(i){
  ...
  empty_index <- get_empty(tab, cols_to_ignore = c("Notes", "Comment", "Taxa|Class|Order|Family|Genus|Species", "OutbreakName"))
  ...
  # check ID dups 
  dup_fields <- c("EventName", "AnimalID", "SpecimenID", "Q1 ParticipantID", rep("ParticipantID", 10))
