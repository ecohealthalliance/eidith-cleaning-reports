...
condition_check_all <- read_csv(h("condition_check_v2.csv")) %>%
...
for (i in seq_along(tables)){
  ...
  create_unique_table(tab, metanames, condition_check = filter(condition_check_all, module_with_na == tab_name),
                      cols_to_ignore = c("Notes", "Comment", "SiteName", "ID", "EventName", "^Class$", "^Order$", "^Family$", "^Genus$", "Species", "Latitude", "Longitude", "TestName"), 
                      do_not_count_empty = c("[Nn]otes", "[Cc]omment", "^Class$", "^Order$", "^Family$", "^Genus$", "Species", "OutbreakName", "InterfacesAnimalsSampled", "InterfacesHumansSampled", "CommunityEngagement", "TagAndOtherID", "[a-zA-Z]Width$", "[a-zA-Z]Height$", "[a-zA-Z]Length$", "^Container$", "StorageMethod", "StorageFacility", "StorageLocation", "LiveStateProv", "LiveLatitude", "LiveLongitude", "LivelihoodGroup\\(s\\)IfOther", "WorkLocation", "TravelledCity Loc [2-6]","Latitude Loc [2-6]", "Longitude Loc [2-6]", "Contact with", "[a-zA-Z]OutbreakDeaths", "Photos")) %>% 
    print()
  cat("</details>")
  cat('\n')
}
...
markup <- map(seq_along(tables), function(i){
  ...
  empty_index <- get_empty(tab, metanames, condition_check = filter(condition_check_all, module_with_na == tab_name), cols_to_ignore = c("[Nn]otes", "[Cc]omment", "^Class$", "^Order$", "^Family$", "^Genus$", "Species", "OutbreakName", "InterfacesAnimalsSampled", "InterfacesHumansSampled", "CommunityEngagement", "TagAndOtherID", "[a-zA-Z]Width$", "[a-zA-Z]Height$", "[a-zA-Z]Length$", "^Container$", "StorageMethod", "StorageFacility", "StorageLocation", "LiveStateProv", "LiveLatitude", "LiveLongitude", "LivelihoodGroup\\(s\\)IfOther", "WorkLocation", "TravelledCity Loc [2-6]","Latitude Loc [2-6]", "Longitude Loc [2-6]", "Contact with", "[a-zA-Z]OutbreakDeaths", "Photos"))
  ...
  if (tab_name == "Event"){
        notmapped_index <- get_notmapped(tab)
  } else {
        notmapped_index <- NULL
  }
  ...
  # check ID dups
  dup_fields <- c("EventName", "AnimalID", "SpecimenID", "TestName", "Q1 ParticipantID", rep("ParticipantID", 10), rep("EventName", 8))
